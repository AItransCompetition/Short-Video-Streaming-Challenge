...
class ActorNetwork2(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.network_params = \
            tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='actor_bitrate')
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor_bitrate'):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])

            reverse_data = tf.transpose(inputs, [0, 2, 1])

            gru_net0 = tflearn.activations.leaky_relu(
                tflearn.layers.recurrent.gru(reverse_data, 16, activation='linear'), alpha=leaky)

            dense_net_0 = tflearn.activations.leaky_relu(tflearn.fully_connected(gru_net0, 32, activation='linear'),
                                                         alpha=leaky)
...
class ActorNetwork1(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # Get all network parameters
        self.network_params = \
            tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='actor_length')
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor_length'):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        with tf.variable_scope('critic'):
            ...
            dense_net_0 = tflearn.activations.leaky_relu(tflearn.fully_connected(gru_net0, 32, activation='linear'),
                                                         alpha=leaky)
            dense_net_1 = tflearn.fully_connected(dense_net_0, A_DIM, activation='linear')
