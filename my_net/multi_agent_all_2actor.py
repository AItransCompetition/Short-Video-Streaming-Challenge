...
def compute_reward(vmaf, last_vmaf, rebuf_size, delay, bitrate, lastbitrate):
    ...
    reward = 10 - VMAF_FACTOR * np.abs((vmaf - VMAF_TARGET) / 5) - REBUF_PENALTY * rebuf_size - np.abs(
        (vmaf - last_vmaf) / 10) - DELAY_PENALTY * delay
    ...
    with tf.Session(config=config) as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        while True:
            ...
            for j in range(1, nreward):
                for i in range(NUM_AGENTS):
                    actor_gradient_batch_length[0][j] = np.add(actor_gradient_batch_length[0][j], actor_gradient_batch_length[i][j])
                    actor_gradient_batch_bitrate[0][j] = np.add(actor_gradient_batch_bitrate[0][j], actor_gradient_batch_bitrate[i][j])
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session(config=config) as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        action_vec1 = np.zeros(A_DIM_LENGTH)
        action_vec2 = np.zeros(A_DIM_BITRATE)
        action_vec1[action1] = 1
        ...
        while True:  # experience video streaming forever
            ...
            if len(r_batch) >= TRAIN_SEQ_LEN or end_of_video:
                exp_queue.put([s_batch[1:],  # ignore the first chuck
                               a_batch_length[1:],  # since we don't have the
                               a_batch_bitrate[1:],
                               r_batch[1:],  # control over it
                               end_of_video,
                               {'entropy1': entropy_record1},
                               {'entropy2': entropy_record2}])
